...
type DropLocationScreenNavigationProp = NativeStackNavigationProp<RootParamList, 'DropLocationScreen'>;
type DropLocationScreenRouteProp = RouteProp<RootParamList, 'DropLocationScreen'>;
...
const route = useRoute<DropLocationScreenRouteProp>();
  const { name, phone, address } = route.params as RouteParams;
  ...
  const getCurrentLocation = async () => {
    try {
      ...
      if (results && results.length > 0) {
        ...
        navigation.navigate('ReceiverDetailsScreen', { location: currentLocation,name:name,address:address,phone:phone });
      } else {
  ...
  const handlePlaceSelect = (data: any, details: any) => {
    if (details && details.geometry) {
      ...
      navigation.navigate('ReceiverDetailsScreen', { location: selectedLocation,name:name,address:address,phone:phone });
    } else {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.buttonGroup}>
        ...
        <TouchableOpacity
          style={[styles.button, { backgroundColor: '#fcba03' }]}
          onPress={() => navigation.navigate('SelectDropOnMapScreen')}
        >
